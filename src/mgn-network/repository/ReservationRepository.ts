...
export class ReservationRepository extends Repository<Reservation> {
    ...
    public async findByUserIdAndBusinessIdIn(userId: number, businessIds: number[], conn = null,  query = null): Promise<Reservation[]> {
        const c = conn;
        // tslint:disable-next-line:max-line-length
        return c.query(query || `
        select * from ${this.table} where user_id = ${mysql2.escape(userId)} and business_id in (?) and deleted_at is null order by id desc`, [businessIds]).then((results) => results);
    }
