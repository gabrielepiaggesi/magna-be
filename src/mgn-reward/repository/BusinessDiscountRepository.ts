...
export class BusinessDiscountRepository extends Repository<BusinessDiscount> {
    ...
    public async findActiveByBusinessIdAndOrigin(businessId: number, origin: 'FIDELITY_CARD'|'IG_POST'|'REFERRAL'|'FIRST_CARD'|'FIRST_RESERVATION'|'FIRST_REVIEW'|'FIRST_ACTION', conn = null,  query = null): Promise<BusinessDiscount> {
        const c = conn;
        // tslint:disable-next-line:max-line-length
        return c.query(query || `
